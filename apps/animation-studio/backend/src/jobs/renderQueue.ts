...
// If the orchestrator provided an enhanced prompt, prefer it
...
async function generateScene(
  ...
  const result = await provider.generateWithFallback({
    prompt: (scene as any).compiledProvider ? getScenePrompt(scene) : enhancePrompt(getScenePrompt(scene), cfg, sceneDurationSec),
    negativePrompt: (scene as any).negativePrompt || undefined,
    referenceImageUrl: (scene as any).referenceImageUrl || undefined,
...
renderQueue.process(config.RENDER_CONCURRENCY, async (job: Job<RenderJobData>) => {
  ...
  try {
    const pipelineResult = await orchestrateAdGeneration({
      renderJobId, workspaceId, userId: job.data.userId,
      brief: renderConfig.brief || `Ad for ${renderConfig.brandName || 'brand'}`,
      brandName: renderConfig.brandName || 'Brand',
      industry: renderConfig.industry || 'Other',
      ...
      maxDurationSec: renderConfig.maxDurationSec || 30,
      brandAssetIds: renderConfig.brandAssetIds,
      brandPalette: renderConfig.brandPalette,
      targetAudience: renderConfig.targetAudience,
      objective: renderConfig.objective,
    });
    if (pipelineResult.storyboard && pipelineResult.storyboard.length === enrichedScenes.length) {
      for (let i = 0; i < enrichedScenes.length; i++) {
        const enhanced = pipelineResult.storyboard[i];
    ...
    logger.warn(`[Worker] AI engine enrichment failed (non-fatal): ${err.message}`, { renderJobId });
  }
