...
export function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-base)", color: "var(--text-primary)", fontFamily: "var(--font-body)" }}>
      ...
      {/* ── Sidebar ── */}
      <aside style={{
        width: collapsed ? 62 : 228, minHeight: "100vh",
        background: "rgba(12,12,22,0.94)", borderRight: "1px solid rgba(255,255,255,0.055)", backdropFilter: "blur(24px)",
        ...
        <div style={{ padding: collapsed ? "10px 6px 4px" : "10px 10px 4px" }}>
          <button onClick={() => setShowGen(true)} style={{
            ...
            background: "linear-gradient(135deg, var(--accent) 0%, #9b5de5 55%, #c084fc 100%)",
            border: "none", borderRadius: "var(--radius-md)", cursor: "pointer",
            color: "#fff", fontSize: 13, fontWeight: 600, fontFamily: "var(--font-body)",
            boxShadow: "var(--shadow-accent)", transition: "all var(--transition-base)",
              ...
              onMouseLeave={e => (e.currentTarget.style.background = "")}>
                ...
                {!collapsed && (
                  <div style={{ flex: 1, textAlign: "left", minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10.5, color: "var(--text-muted)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{user.role}</div>
                  </div>
                )}
              </button>
