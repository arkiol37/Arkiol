...
interface BillingInfo {
  plan: string;
  credits: { remaining: number; used: number; limit: number; usagePct: number };
  cycleEndsAt?: string;
...
const PLAN_COLOR: Record<string, string> = {
  FREE: "var(--text-muted)", CREATOR: "var(--accent-light)",
  PRO: "var(--tertiary)", STUDIO: "var(--secondary)",
  // Legacy enum aliases (schema migration compatibility)
  };
...
export function DashboardHome({ user }: { user?: any }) {
  ...
  return (
    <div className="ak-fade-in" style={{ padding: "36px 44px", maxWidth: 1240 }}>
      {/* Header */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 36 }}>
        <div>
          ...
          <p style={{ margin: "5px 0 0", fontSize: 13, color: "var(--text-muted)", letterSpacing: "-0.01em" }}>
            Here&apos;s your Arkiol workspace at a glance.
          </p>
        </div>
      ...
      {isPastDue && (
        <div className="ak-toast ak-toast-error" style={{ marginBottom: 24, fontSize: 13 }}>
          <span>⚠</span>
          <span>Payment failed. <a href="/billing" style={{ color: "#fca5a5", textDecoration: "underline" }}>Update billing details</a> to keep your account active.</span>
        </div>
      )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(210px, 1fr))", gap: 14, marginBottom: 32 }}>
          {[
            ...
            },
            {
              ...
              sub: <a href="/billing" style={{ color: "var(--accent-light)", textDecoration: "none", fontSize: 12 }}>{["FREE","CREATOR"].includes(billing?.plan ?? "") ? "Upgrade plan →" : "Manage →"}</a>,
              accent: undefined,
            },
            {
            ...
            <div key={i} className="ak-card ak-stat-card" style={{ padding: "22px 24px" }}>
              ...
              <div style={{ fontSize: 34, fontWeight: 800, fontFamily: "var(--font-display)", letterSpacing: "-0.05em", color: (s as any).accent ?? "var(--text-primary)", lineHeight: 1 }}>{s.value}</div>
      ...
      {!loading && recentJobs.length === 0 && (
        <div style={{ background: "var(--bg-elevated)", borderRadius: "var(--radius-2xl)", border: "1px dashed rgba(124,127,250,0.25)", overflow: "hidden" }}>
          {/* Empty state header */}
          <div style={{ textAlign: "center", padding: "44px 24px 28px" }}>
            ...
            <h3 style={{ fontSize: 20, fontWeight: 800, fontFamily: "var(--font-display)", margin: "0 0 10px", letterSpacing: "-0.04em" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", borderTop: "1px solid var(--border)" }}>
            {[
              { n: "1", label: "Enter prompt",       icon: "◎", color: "#7c7ffa" },
              { n: "2", label: "Generate designs",   icon: "✦", color: "#9b5de5" },
              { n: "3", label: "Select result",      icon: "◈", color: "#f472b6" },
              { n: "4", label: "Export or edit",     icon: "◉", color: "#22d3ee" },
            ].map((s, i) => (
              <div key={s.n} style={{
