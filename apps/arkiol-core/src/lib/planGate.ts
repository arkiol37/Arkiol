...
export async function loadOrgSnapshot(orgId: string): Promise<OrgEnforcementSnapshot> {
...
export async function assertZipExportAllowed(orgId: string): Promise<void> {
  const snap = await loadOrgSnapshot(orgId);
  assertEnforcement(checkZipExport(snap));
}

// Check for Studio access
export async function assertStudioAccessAllowed(orgId: string): Promise<void> {
  const snap = await loadOrgSnapshot(orgId);
...
export async function assertBatchAllowed(orgId: string, requestedJobs: number): Promise<OrgEnforcementSnapshot> {
  const snap = await loadOrgSnapshot(orgId);
