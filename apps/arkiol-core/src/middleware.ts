...
async function middleware(req: NextRequest): Promise<NextResponse> {
  ...
  // ── CSRF guard ──────────────────────────────────────────────────────────────
  // Derives allowed origin from the request itself so any domain (production,
  // preview, custom) works without hardcoded env vars. An origin is valid when
  // its host matches the host serving this request.
  if (
    ...
    if (origin) {
      // Extract just the host from the incoming origin header
      let originHost: string;
      try { originHost = new URL(origin).host; }
      catch { originHost = ''; }

      // The canonical host is the request's own host (works on any deployment)
      const requestHost = req.headers.get('host') ?? req.nextUrl.host;

      // Also allow the configured production domain as explicit allowlist
      const configuredUrl = process.env.NEXTAUTH_URL ?? process.env.NEXT_PUBLIC_APP_URL ?? '';
      let configuredHost = '';
      if (configuredUrl) {
        try { configuredHost = new URL(configuredUrl).host; } catch {}
      }

      const isAllowed =
        !originHost ||                           // no origin host → allow (same-origin curl etc.)
        originHost === requestHost ||             // matches request host → same-origin ✓
        (configuredHost && originHost === configuredHost); // matches configured canonical domain ✓

      if (!isAllowed) {
        console.warn(`[csrf] blocked origin=${origin} host=${requestHost} path=${pathname}`);
