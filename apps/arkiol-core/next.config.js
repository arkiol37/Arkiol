...
const nextConfig = {
  ...
  // Lint is enforced in CI (see .github/workflows/ci.yml lint job).
  // ignoreDuringBuilds avoids re-running ESLint during `next build` since CI already covers it.
  eslint:     { ignoreDuringBuilds: true },

  // Type-checking is enforced in CI via `tsc --noEmit` (see ci.yml typecheck job).
  // ignoreBuildErrors prevents `next build` from re-running tsc, which would
  // double the build time on Vercel. If CI typecheck passes, this is safe.
